...
echo "Jazz: User: $USER" 
echo "Jazz: User home: $USER_HOME"
echo "Jazz: Setup scripts: $SETUP_SCRIPTS"
echo "======================================================"

echo "Jazz: Update system..."
apt-get update -y
#apt-get upgrade -y

echo "Jazz: Install git..."
...
function cloneScriptsDir() {
    ...
    if [ -d "$SETUP_SCRIPTS" ]
    then
        echo "Jazz: Remove scripts folder..."
        rm -rf $SETUP_SCRIPTS
    fi
    
    # clone fresh copy from github
    echo "Jazz: clone setup-scripts from github"
...
# run the root script
echo "Jazz: Execute run_as_root"
cd $SETUP_SCRIPTS && cd "jazz" && ./run_as_root.sh > run_as_root.log 2>&1

# run the user script
echo "Jazz: Execute run_as_user"
/bin/su - $USER -c "cd $SETUP_SCRIPTS && cd jazz && ./run_as_user.sh > run_as_user.log 2>&1"
