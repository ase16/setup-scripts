...
echo "Carlton: User: $USER" 
echo "Carlton: User home: $USER_HOME"
echo "Carlton: Setup scripts: $SETUP_SCRIPTS"
echo "======================================================"

echo "Carlton: Update system..."
apt-get update -y
#apt-get upgrade -y

echo "Carlton: Install git..."
...
function cloneScriptsDir() {
    ...
    if [ -d "$SETUP_SCRIPTS" ]
    then
        echo "Carlton: Remove scripts folder..."
        rm -rf $SETUP_SCRIPTS
    fi
    
    # clone fresh copy from github
    echo "Carlton: clone setup-scripts from github"
...
# run the root script
echo "Carlton: Execute run_as_root"
cd $SETUP_SCRIPTS && cd "carlton" && ./run_as_root.sh > run_as_root.log 2>&1

# run the user script
echo "Carlton: Execute run_as_user"
/bin/su - $USER -c "cd $SETUP_SCRIPTS && cd carlton && ./run_as_user.sh > run_as_user.log 2>&1"
