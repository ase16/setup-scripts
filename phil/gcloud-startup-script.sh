...
echo "Phil: User: $USER" 
echo "Phil: User home: $USER_HOME"
echo "Phil: Setup scripts: $SETUP_SCRIPTS"
echo "======================================================"

echo "Phil: Update system..."
apt-get update -y
#apt-get upgrade -y

echo "Phil: Install git..."
...
function cloneScriptsDir() {
    ...
    if [ -d "$SETUP_SCRIPTS" ]
    then
        echo "Phil: Remove scripts folder..."
        rm -rf $SETUP_SCRIPTS
    fi
    
    # clone fresh copy from github
    echo "Phil: clone setup-scripts from github"
...
# run the root script
echo "Phil: Execute run_as_root"
cd $SETUP_SCRIPTS && cd "phil" && ./run_as_root.sh > run_as_root.log 2>&1

# run the user script
echo "Phil: Execute run_as_user"
/bin/su - $USER -c "cd $SETUP_SCRIPTS && cd phil && ./run_as_user.sh > run_as_user.log 2>&1"
